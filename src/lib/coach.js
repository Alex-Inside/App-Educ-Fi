...
const TOPICS = [
  {
    keywords: ['découvert', 'decouvert', 'agios', 'négatif', 'negatif'],
    reply:
  ...
  {
    keywords: ['50/30/20', '50 30 20', 'budget', 'dépense', 'depense', 'fin de mois'],
    reply:
      "La règle 50/30/20 découpe ton revenu en 3 enveloppes : 50 % pour l'essentiel (loyer, courses, transport), 30 % pour le plaisir, 20 % que tu mets de côté dès que l'argent arrive. L'astuce qui change tout : un virement automatique vers l'épargne le lendemain de la paie.",
  },
  {
    keywords: ['livret a', 'livret'],
    reply:
      "Le Livret A, c'est le coffre de secours idéal : zéro risque, argent disponible à tout moment, et les intérêts sont totalement défiscalisés. Son rôle, c'est l'épargne de précaution (2-3 mois de dépenses) — pas le long terme, pour ça il y a de meilleurs outils (module 4 👀).",
  },
  {
    keywords: ['pel', 'plan épargne logement', 'plan epargne logement'],
    reply:
  ...
  {
    keywords: ['intérêts composés', 'interets composes', 'composé', 'compose', 'boule de neige'],
    reply:
      "Les intérêts composés, c'est l'effet boule de neige : les intérêts gagnés produisent eux-mêmes des intérêts. 100 € à 10 % → 110 €, puis les 10 % se calculent sur 110 €… Le facteur le plus puissant n'est pas le taux, c'est la durée. C'est pour ça que commencer à 22 ans change tout par rapport à 32.",
  },
  {
    keywords: ['précaution', 'precaution', 'urgence', 'imprévu', 'imprevu', 'combien épargner', 'combien epargner'],
    reply:
  ...
  {
    keywords: ['taeg', 'taux', 'crédit', 'credit', 'emprunt', 'prêt', 'pret'],
    reply:
  ...
  {
    keywords: ['plusieurs fois', '3 fois', '4 fois', '36 fois', 'renouvelable', 'paiement fractionné', 'paiement fractionne'],
    reply:
  ...
  {
    keywords: ['pea', "plan d'épargne en actions", 'plan epargne actions'],
    reply:
  ...
  {
    keywords: ['fiche de paie', 'brut', 'salaire net', 'cotisation'],
    reply:
  ...
  {
    keywords: ['aide', 'apl', 'caf', 'bourse', 'prime d’activité', "prime d'activité", 'crous'],
    reply:
  ...
  {
    keywords: ['assurance habitation', 'responsabilité civile', 'responsabilite civile', 'coloc', 'assurance auto', 'jeune conducteur', 'bonus'],
    reply:
  ...
  {
    keywords: ['arnaque', 'scam', 'finfluenceur', 'ponzi', 'pyramide', 'faux conseiller', 'phishing', 'fraude'],
    reply:
  ...
  {
    keywords: ['dérivé', 'derive', 'option', 'future', 'cfd', 'turbo', 'warrant', 'levier', 'short', 'vente à découvert'],
    reply:
  ...
  {
    keywords: ['private equity', 'non coté', 'non-coté', 'crowdfunding', 'fcpi', 'fip', 'startup'],
    reply:
...
const OFF_TOPIC = [
  'foot', 'film', 'série', 'serie', 'météo', 'meteo', 'recette', 'jeu vidéo', 'jeux video',
...
export function getCoachGreeting({ profile, moduleId }) {
  ...
  return `Salut ! 👋 Je suis ton coach FinPath. ${ctx}`
}
...
export function getCoachReply({ text, profile, moduleId }) {
  ...
  if (OFF_TOPIC.some((w) => q.includes(w))) {
    return "Ah, ça c'est pas mon rayon 😄 Moi je suis là pour tes questions d'argent — budget, épargne, crédit, investissement… Il y a un sujet qui te tracasse en ce moment côté finances ?"
  }
  ...
  const suggestion = moduleId
    ...
    : 'Essaie de me la poser avec un mot-clé : découvert, Livret A, TAEG, ETF, PEA, assurance-vie…'
  return `Bonne question ! Pour l'instant je tourne en mode démo (l'API Claude n'est pas encore branchée), donc je réponds surtout sur les grands sujets du parcours. ${suggestion}`
}
